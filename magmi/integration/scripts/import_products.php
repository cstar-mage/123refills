...
$dp->beginImportSession("123_import_products", "xcreate");
...
foreach($lines as $column)
{
	...
	//John said: this dont need per product, so use anil's tool every time
	//$item['url_key'] = Mage::getModel('catalog/product_url')->formatUrlKey($name.'-'.$sku); //for unique url

	$item['description'] = $description;
	$item['short_description'] = $sdescription;
	$item['weight'] = $weight;
	$item['price'] = $price;
