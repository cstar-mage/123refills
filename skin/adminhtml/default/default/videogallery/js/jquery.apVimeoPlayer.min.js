	 ...
	 function ap_VimeoPlayer(data){
		...
		this._player.addEvent('ready', function(){
			...
			_self._player.addEvent('play', function(data) {
				//console.log('ap_VimeoPlayer.VIMEO_PLAY');
				$(_self).trigger('ap_VimeoPlayer.VIMEO_PLAY');	
				self.playing=true;
				//if(_self.isMobile && _self.initialAutoplay)_self._autoPlay=true;//THIS DOESNT WORK ON IOS!! (breaks playback)
				if(!_self.isMobile)_self._autoPlay=true;
			});
			_self._player.addEvent('pause', function(data) {
				//console.log('ap_VimeoPlayer.VIMEO_PAUSE');
