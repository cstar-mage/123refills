<?php 
class Varien_Data_Form_Element_Gallery11 extends Varien_Data_Form_Element_Abstract
{
    ...
    public function getElementHtml()
    {
        	...
        	$query = "SELECT * FROM ".$table_name." WHERE `shape` = 'Round' ";
           ...
           $caratweightJson = json_encode($caratweight);
            ...
            $html = "<input type='hidden' value='".$caratrowcount."' id='roundcount' name='roundcount' />
				<table width='100%' id='roundtable'>
			...
			foreach ($newArray as $caratkey => $caratarray) {
				...
				$html .= "<tr id='round-row-".$i."'>
						 <td><p class='caratvalue'>".$caratkey."</p><input type='hidden' class='validate-number' name='round-".$i."[]' value=".$caratkey." /><input type='hidden' name='round-".$i."[]' value='Round' />                         
                         </td>";
				$a=0;
				$columncount = 2;
				...
				foreach($caratarray as $key => $value){				
                    ...
                    $key1 = array_search($value['dbfield'],$k3);
                		...
                		$html .= "<td><input type='text' class='validate-number required-entry' name='round-".$i."[]' value=".$value['price']." />                       
                        <input type='hidden' name='round-".$i."[]' value=".$value['diamondstud_id']." />
                        <input type='hidden' name='round-".$i."[]' value=".$value['dbfield']." />
                        </td>";
				...
				if($a != $rowcount){
                    ...
                    $p--;
                     ...
                     for($n=0;$n<$remainrow;$n++){
						...
						$html .= "<td><input type='text' class='validate-number required-entry' name='round-".$i."[]' value='' /><input type='hidden' name='round-".$i."[]' value='' />
                        <input type='hidden' name='round-".$i."[]' value='".$hhh[$n]."' />
                        </td>";
       ...
       $html .= '<thead id="gallery_thead_2" class="gallery"></thead>';
        ...
        var caratfinalround = caratarray.filter(function(val) {return caratexist1.indexOf(val) == -1;});

        var caratfcountround = caratfinalround.length;
              
        catarray = catarray.reverse();

        roundid2 = $caratrowcount+1; 
		...
		document.getElementById("roundcount").value = roundid2;
    	var row = table.insertRow(roundid2);
		row.id = 'round-row-'+ roundid2;
        ...
        for(j=rowid;j>=1;j--){
           ...
           if(j==1){
            ...
            for(k=0;k<caratfcountround;k++){
                aaa += "<option value="+caratfinalround[k]+">"+caratfinalround[k]+"</option>";  
            }
          
             cell1.innerHTML = "<select style='width:157px' name='round-"+roundid2+"[]'>"+aaa+"</select><input type='hidden' value='Round' name='round-"+roundid2+"[]'/>";
        //    cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='round-"+roundid2+"[]'/><input type='hidden' value='Round' name='round-"+roundid2+"[]'/>"; 

           }else{

            var cell1 = 'cell'+j;
            var cell1 = row.insertCell(0);
            cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='round-"+roundid2+"[]'/><input type='hidden' name='round-"+roundid2+"[]'/><input type='hidden' value='"+catarray[t]+"' name='round-"+roundid2+"[]'/>"; 
            t++;
           }
