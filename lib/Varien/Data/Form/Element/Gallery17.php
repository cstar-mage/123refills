...
<?php 
class Varien_Data_Form_Element_Gallery17 extends Varien_Data_Form_Element_Abstract
{
    ...
    public function getElementHtml()
    {
            ...
            $query = "SELECT * FROM ".$table_name." WHERE `shape` = 'RadiantLong' ";
           ...
           $caratweightJson = json_encode($caratweight);
            ...
            $html = "<input type='hidden' value='".$caratrowcount."' id='RadiantLongcount' name='RadiantLongcount' />
                <table width='100%' id='RadiantLongtable'>
            ...
            foreach ($newArray as $caratkey => $caratarray) {
                ...
                $html .= "<tr id='RadiantLong-row-".$i."'>
                                         <td><p class='caratvalue'>".$caratkey."</p><input type='hidden' class='validate-number' name='RadiantLong-".$i."[]' value=".$caratkey." /><input type='hidden' name='RadiantLong-".$i."[]' value='RadiantLong' />                         
                         </td>";
                $a=0;
                $columncount = 2;
                ...
                foreach($caratarray as $key => $value){             
                    ...
                    $key1 = array_search($value['dbfield'],$k3);
                        ...
                        $html .= "<td><input type='text' class='validate-number required-entry' name='RadiantLong-".$i."[]' value=".$value['price']." />                       
                        <input type='hidden' name='RadiantLong-".$i."[]' value=".$value['diamondstud_id']." />
                        <input type='hidden' name='RadiantLong-".$i."[]' value=".$value['dbfield']." />
                        </td>";
                ...
                if($a != $rowcount){
                    ...
                    $p--;
                     ...
                     for($n=0;$n<$remainrow;$n++){
                        ...
                        $html .= "<td><input type='text' class='validate-number required-entry' name='RadiantLong-".$i."[]' value='' /><input type='hidden' name='RadiantLong-".$i."[]' value='' />
                        <input type='hidden' name='RadiantLong-".$i."[]' value='".$hhh[$n]."' />
                        </td>";
       ...
       $html .= '<thead id="gallery_thead_2" class="gallery"></thead>';
        ...
        $buttonHtml = $widgetButton->createBlock('adminhtml/widget_button')
                ->setData(
                    array(
                        'label'     => 'Add RadiantLong Price',
                        'onclick'   => 'addRadiantLong()',
                        'class'     => 'add'))
                ->toHtml();
        ...
        var caratfinalRadiantLong = caratarray.filter(function(val) {return caratexist1.indexOf(val) == -1;});

            


        var caratfcountRadiantLong = caratfinalRadiantLong.length;
              
        catarray = catarray.reverse();

        RadiantLongid2 = $caratrowcount+1; 
        ...
        function addRadiantLong(){
        
        var table = document.getElementById("RadiantLongtable");
        document.getElementById("RadiantLongcount").value = RadiantLongid2;
        var row = table.insertRow(RadiantLongid2);
        row.id = 'RadiantLong-row-'+ RadiantLongid2;
        ...
        for(j=rowid;j>=1;j--){
           ...
           if(j==1){
            ...
            for(k=0;k<caratfcountRadiantLong;k++){
                aaa += "<option value="+caratfinalRadiantLong[k]+">"+caratfinalRadiantLong[k]+"</option>";  
            }
          
             cell1.innerHTML = "<select style='width:157px' name='RadiantLong-"+RadiantLongid2+"[]'>"+aaa+"</select><input type='hidden' value='RadiantLong' name='RadiantLong-"+RadiantLongid2+"[]'/>";
        //    cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='RadiantLong-"+RadiantLongid2+"[]'/><input type='hidden' value='RadiantLong' name='RadiantLong-"+RadiantLongid2+"[]'/>"; 

           }else{

            var cell1 = 'cell'+j;
            var cell1 = row.insertCell(0);
            cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='RadiantLong-"+RadiantLongid2+"[]'/><input type='hidden' name='RadiantLong-"+RadiantLongid2+"[]'/><input type='hidden' value='"+catarray[t]+"' name='RadiantLong-"+RadiantLongid2+"[]'/>"; 
            t++;
           }
