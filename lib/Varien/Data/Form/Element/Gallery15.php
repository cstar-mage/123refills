...
<?php 
class Varien_Data_Form_Element_Gallery15 extends Varien_Data_Form_Element_Abstract
{
    ...
    public function getElementHtml()
    {
            ...
            $query = "SELECT * FROM ".$table_name." WHERE `shape` = 'Asscher' ";
           ...
           $caratweightJson = json_encode($caratweight);
            ...
            $html = "<input type='hidden' value='".$caratrowcount."' id='asschercount' name='asschercount' />
                <table width='100%' id='asschertable'>
            ...
            foreach ($newArray as $caratkey => $caratarray) {
                ...
                $html .= "<tr id='asscher-row-".$i."'>
               <td><p class='caratvalue'>".$caratkey."</p><input type='hidden' class='validate-number' name='asscher-".$i."[]' value=".$caratkey." /><input type='hidden' name='asscher-".$i."[]' value='asscher' />                         
                         </td>";
                $a=0;
                $columncount = 2;
                ...
                foreach($caratarray as $key => $value){             
                    ...
                    $key1 = array_search($value['dbfield'],$k3);
                        ...
                        $html .= "<td><input type='text' class='validate-number required-entry' name='asscher-".$i."[]' value=".$value['price']." />                       
                        <input type='hidden' name='asscher-".$i."[]' value=".$value['diamondstud_id']." />
                        <input type='hidden' name='asscher-".$i."[]' value=".$value['dbfield']." />
                        </td>";
                ...
                if($a != $rowcount){
                    ...
                    $p--;
                     ...
                     for($n=0;$n<$remainrow;$n++){
                        ...
                        $html .= "<td><input type='text' class='validate-number required-entry' name='asscher-".$i."[]' value='' /><input type='hidden' name='asscher-".$i."[]' value='' />
                        <input type='hidden' name='asscher-".$i."[]' value='".$hhh[$n]."' />
                        </td>";
       ...
       $html .= '<thead id="gallery_thead_2" class="gallery"></thead>';
        ...
        var caratfinalasscher = caratarray.filter(function(val) {return caratexist1.indexOf(val) == -1;});

            


        var caratfcountasscher = caratfinalasscher.length;
              
        catarray = catarray.reverse();

        asscherid2 = $caratrowcount+1; 
        ...
        var table = document.getElementById("asschertable");
        document.getElementById("asschercount").value = asscherid2;
        var row = table.insertRow(asscherid2);
        row.id = 'asscher-row-'+ asscherid2;
        ...
        for(j=rowid;j>=1;j--){
           ...
           if(j==1){
            ...
            for(k=0;k<caratfcountasscher;k++){
                aaa += "<option value="+caratfinalasscher[k]+">"+caratfinalasscher[k]+"</option>";  
            }
          
             cell1.innerHTML = "<select style='width:157px' name='asscher-"+asscherid2+"[]'>"+aaa+"</select><input type='hidden' value='asscher' name='asscher-"+asscherid2+"[]'/>";
        //    cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='asscher-"+asscherid2+"[]'/><input type='hidden' value='asscher' name='asscher-"+asscherid2+"[]'/>"; 

           }else{

            var cell1 = 'cell'+j;
            var cell1 = row.insertCell(0);
            cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='asscher-"+asscherid2+"[]'/><input type='hidden' name='asscher-"+asscherid2+"[]'/><input type='hidden' value='"+catarray[t]+"' name='asscher-"+asscherid2+"[]'/>"; 
            t++;
           }
