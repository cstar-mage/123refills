...
<?php 
class Varien_Data_Form_Element_Gallery26 extends Varien_Data_Form_Element_Abstract
{
    ...
    public function getElementHtml()
    {
            ...
            $query = "SELECT * FROM ".$table_name." WHERE `shape` = 'Oval' ";
           ...
           $caratweightJson = json_encode($caratweight);
            ...
            $html = "<input type='hidden' value='".$caratrowcount."' id='ovalcount' name='ovalcount' />
                <table width='100%' id='ovaltable'>
            ...
            foreach ($newArray as $caratkey => $caratarray) {
                ...
                $html .= "<tr id='oval-row-".$i."'>
                                             <td><p class='caratvalue'>".$caratkey."</p><input type='hidden' class='validate-number' name='oval-".$i."[]' value=".$caratkey." /><input type='hidden' name='oval-".$i."[]' value='oval' />                         
                         </td>";
                $a=0;
                $columncount = 2;
                ...
                foreach($caratarray as $key => $value){             
                    ...
                    $key1 = array_search($value['dbfield'],$k3);
                        ...
                        $html .= "<td><input type='text' class='validate-number required-entry' name='oval-".$i."[]' value=".$value['price']." />                       
                        <input type='hidden' name='oval-".$i."[]' value=".$value['diamondstud_id']." />
                        <input type='hidden' name='oval-".$i."[]' value=".$value['dbfield']." />
                        </td>";
                ...
                if($a != $rowcount){
                    ...
                    $p--;
                     ...
                     for($n=0;$n<$remainrow;$n++){
                        ...
                        $html .= "<td><input type='text' class='validate-number required-entry' name='oval-".$i."[]' value='' /><input type='hidden' name='oval-".$i."[]' value='' />
                        <input type='hidden' name='oval-".$i."[]' value='".$hhh[$n]."' />
                        </td>";
       ...
       $html .= '<thead id="gallery_thead_2" class="gallery"></thead>';
        ...
        var caratfinaloval = caratarray.filter(function(val) {return caratexist1.indexOf(val) == -1;});

            


        var caratfcountoval = caratfinaloval.length;
              
        catarray = catarray.reverse();

        ovalid2 = $caratrowcount+1; 
        ...
        var table = document.getElementById("ovaltable");
        document.getElementById("ovalcount").value = ovalid2;
        var row = table.insertRow(ovalid2);
        row.id = 'oval-row-'+ ovalid2;
        ...
        for(j=rowid;j>=1;j--){
           ...
           if(j==1){
            ...
            for(k=0;k<caratfcountoval;k++){
                aaa += "<option value="+caratfinaloval[k]+">"+caratfinaloval[k]+"</option>";  
            }
          
             cell1.innerHTML = "<select style='width:157px' name='oval-"+ovalid2+"[]'>"+aaa+"</select><input type='hidden' value='oval' name='oval-"+ovalid2+"[]'/>";
        //    cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='oval-"+ovalid2+"[]'/><input type='hidden' value='oval' name='oval-"+ovalid2+"[]'/>"; 

           }else{

            var cell1 = 'cell'+j;
            var cell1 = row.insertCell(0);
            cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='oval-"+ovalid2+"[]'/><input type='hidden' name='oval-"+ovalid2+"[]'/><input type='hidden' value='"+catarray[t]+"' name='oval-"+ovalid2+"[]'/>"; 
            t++;
           }
