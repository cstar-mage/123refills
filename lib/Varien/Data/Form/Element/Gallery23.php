...
<?php 
class Varien_Data_Form_Element_Gallery23 extends Varien_Data_Form_Element_Abstract
{
    ...
    public function getElementHtml()
    {
            ...
            $query = "SELECT * FROM ".$table_name." WHERE `shape` = 'Marquise' ";
           ...
           $caratweightJson = json_encode($caratweight);
            ...
            $html = "<input type='hidden' value='".$caratrowcount."' id='marquisecount' name='marquisecount' />
                <table width='100%' id='marquisetable'>
            ...
            foreach ($newArray as $caratkey => $caratarray) {
                ...
                $html .= "<tr id='marquise-row-".$i."'>
                                         <td><p class='caratvalue'>".$caratkey."</p><input type='hidden' class='validate-number' name='marquise-".$i."[]' value=".$caratkey." /><input type='hidden' name='marquise-".$i."[]' value='marquise' />                         
                         </td>";
                $a=0;
                $columncount = 2;
                ...
                foreach($caratarray as $key => $value){             
                    ...
                    $key1 = array_search($value['dbfield'],$k3);
                        ...
                        $html .= "<td><input type='text' class='validate-number required-entry' name='marquise-".$i."[]' value=".$value['price']." />                       
                        <input type='hidden' name='marquise-".$i."[]' value=".$value['diamondstud_id']." />
                        <input type='hidden' name='marquise-".$i."[]' value=".$value['dbfield']." />
                        </td>";
                ...
                if($a != $rowcount){
                    ...
                    $p--;
                     ...
                     for($n=0;$n<$remainrow;$n++){
                        ...
                        $html .= "<td><input type='text' class='validate-number required-entry' name='marquise-".$i."[]' value='' /><input type='hidden' name='marquise-".$i."[]' value='' />
                        <input type='hidden' name='marquise-".$i."[]' value='".$hhh[$n]."' />
                        </td>";
       ...
       $html .= '<thead id="gallery_thead_2" class="gallery"></thead>';
        ...
        var caratfinalmarquise = caratarray.filter(function(val) {return caratexist1.indexOf(val) == -1;});

            


        var caratfcountmarquise = caratfinalmarquise.length;
              
        catarray = catarray.reverse();

        marquiseid2 = $caratrowcount+1; 
        ...
        function addmarquise(){
        
        var table = document.getElementById("marquisetable");
        document.getElementById("marquisecount").value = marquiseid2;
        var row = table.insertRow(marquiseid2);
        row.id = 'marquise-row-'+ marquiseid2;
        ...
        for(j=rowid;j>=1;j--){
           ...
           if(j==1){
            ...
            for(k=0;k<caratfcountmarquise;k++){
                aaa += "<option value="+caratfinalmarquise[k]+">"+caratfinalmarquise[k]+"</option>";  
            }
          
             cell1.innerHTML = "<select style='width:157px' name='marquise-"+marquiseid2+"[]'>"+aaa+"</select><input type='hidden' value='marquise' name='marquise-"+marquiseid2+"[]'/>";
        //    cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='marquise-"+marquiseid2+"[]'/><input type='hidden' value='marquise' name='marquise-"+marquiseid2+"[]'/>"; 

           }else{

            var cell1 = 'cell'+j;
            var cell1 = row.insertCell(0);
            cell1.innerHTML = "<input type='text' class='validate-number required-entry' name='marquise-"+marquiseid2+"[]'/><input type='hidden' name='marquise-"+marquiseid2+"[]'/><input type='hidden' value='"+catarray[t]+"' name='marquise-"+marquiseid2+"[]'/>"; 
            t++;
           }
