...
class Mage_Eternity_Adminhtml_EternityController extends Mage_Adminhtml_Controller_action
{	
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu("eternity/items")
	...
	public function editAction() {
		...
		$model  = Mage::getModel("eternity/eternity")->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register("eternity_data", $model);

			$this->loadLayout();
			$this->_setActiveMenu("eternity/items");
			...
			$this->_addContent($this->getLayout()->createBlock("eternity/adminhtml_eternity_edit"))
				->_addLeft($this->getLayout()->createBlock("eternity/adminhtml_eternity_edit_tabs"));

			$this->renderLayout();
		} else {
			Mage::getSingleton("adminhtml/session")->addError(Mage::helper("eternity")->__("Item does not exist"));
			$this->_redirect("*/*/");
		}
	}
	...
	public function saveAction() {	
		...
		try
		{
			...
			$config -> saveConfig('eternity/general_settings/eternitycolor', $post["eternitycolor"] , 'default', $storeId);
			$config -> saveConfig('eternity/general_settings/eternityfontcolor', $post["eternityfontcolor"] , 'default', $storeId);
			$config -> saveConfig('eternity/general_settings/eternityactivefontcolor', $post["eternityactivefontcolor"] , 'default', $storeId);
			
			
			for($i=0;$i<count($_REQUEST['applied_rule']);$i++){	
				if($_REQUEST['applied_rule'][$i]['id'] != ""){					
					$model = Mage::getModel('eternity/appliedrule')->load($_REQUEST['applied_rule'][$i]['id'])->addData($_REQUEST['applied_rule'][$i]);
					$model->setId($_REQUEST['applied_rule'][$i]['id'])->save();
				}else{		
					Mage::getModel('eternity/appliedrule')->setData($_REQUEST['applied_rule'][$i])->save();
				}
			}
			
			for($j=0;$j<count($_REQUEST['dia_price']);$j++){
			    if($_REQUEST['dia_price'][$j]['id'] != ""){
					$model = Mage::getModel('eternity/diaprice')->load($_REQUEST['dia_price'][$j]['id'])->addData($_REQUEST['dia_price'][$j]);
					$model->setId($_REQUEST['dia_price'][$j]['id'])->save();
				}else{
					Mage::getModel('eternity/diaprice')->setData($_REQUEST['dia_price'][$j])->save();
				}
			}
			
			for($k=0;$k<count($_REQUEST['ring_cost']);$k++){
				
				if($_REQUEST['ring_cost'][$k]['id'] != ""){
					$model = Mage::getModel('eternity/ringcost')->load($_REQUEST['ring_cost'][$k]['id'])->addData($_REQUEST['ring_cost'][$k]);
					$model->setId($_REQUEST['ring_cost'][$k]['id'])->save();
				}else{
					Mage::getModel('eternity/ringcost')->setData($_REQUEST['ring_cost'][$k])->save();
				}
			}
			
			for($l=0;$l<count($_REQUEST['stone_qty']);$l++){
				if($_REQUEST['stone_qty'][$l]['id'] != ""){
					$model = Mage::getModel('eternity/stoneqty')->load($_REQUEST['stone_qty'][$l]['id'])->addData($_REQUEST['stone_qty'][$l]);
					$model->setId($_REQUEST['stone_qty'][$l]['id'])->save();
				}else{
					Mage::getModel('eternity/stoneqty')->setData($_REQUEST['stone_qty'][$l])->save();
				}
			}
	...
	public function insertinpopupAction()
	{
		...
		try
		{
			...
			$user = Mage::getStoreConfig('eternity/user_detail/rapnet_username');
			$passwd = Mage::getStoreConfig('eternity/user_detail/rapnet_password');
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam("id") > 0 ) {
			try {
				$model = Mage::getModel("eternity/eternity");
    ...
    public function massDeleteAction() {
        $eternityIds = $this->getRequest()->getParam("eternity");
        if(!is_array($eternityIds)) {
			Mage::getSingleton("adminhtml/session")->addError(Mage::helper("adminhtml")->__("Please select item(s)"));
        } else {
            try {
                foreach ($eternityIds as $eternityId) {
                    $eternity = Mage::getModel("eternity/eternity")->load($eternityId);
                    $eternity->delete();
                }
                Mage::getSingleton("adminhtml/session")->addSuccess(
                    Mage::helper("adminhtml")->__(
                        "Total of %d record(s) were successfully deleted", count($eternityIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton("adminhtml/session")->addError($e->getMessage());
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($eternityIds)) {
            ...
            try {
                foreach ($eternityIds as $eternityId) {
                    $eternity = Mage::getSingleton("eternity/eternity")
                        ->load($eternityId)
                ...
                $this->_getSession()->addSuccess(
                    $this->__("Total of %d record(s) were successfully updated", count($eternityIds))
                );
            } catch (Exception $e) {
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock("eternity/adminhtml_eternity_grid")
            ->getCsv();
    ...
    {
    	return Mage::getSingleton('admin/session')->isAllowed('catalog/eternity');
    }
}