	...
	class GoMage_Navigation_Model_Search_Layer extends GoMage_Navigation_Model_Layer {
	    ...
	    public function getProductCollection()
	    {
	        if (isset($this->_productCollections[$this->getCurrentCategory()->getId()])) {
	            ...
	            if($this->checkIfCategoryQueryMatch()) {
	            	$collection = $this->checkIfCategoryQueryMatch();
	            }
	    ...
	    public function checkIfCategoryQueryMatch($collection) {
	    	...
	    	if($queryText) {
		    	$_category = Mage::getModel('catalog/category')->getCollection()
					    	->addAttributeToFilter('name', $queryText)
					    	->getFirstItem();
		    	
		    	// we have all grouped products assigned to category so just load matched category products
		    	if($_category->getId()) {
		    		
		    		//only for brand category. parent id is root (id 2) or More Brands (id 15)
		    		if($_category->getParentId() == 2 || $_category->getParentId() == 15) {
			    		$collection = Mage::getModel('catalog/category')->load($_category->getId())->getProductCollection();
			    		return $collection;
		    		}
		    	}
		    }
	    ...
	    public function prepareProductCollection($collection)
	    {
	        ...
	        // if category group show then no need to filter search parameters.
	        if(!$this->checkIfCategoryQueryMatch()) {
