 ...
 */
class Mfmc_Mfmcallforprice_AttributeController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        ...
        try {
            if ($attributesData) {
				...
				if(isset($attributesData['mfm_cfp']) && $attributesData['mfm_cfp']!=''){
					foreach($this->_getHelper()->getProductIds() as $proId){
						$item = Mage::getModel('mfmcallforprice/mfmcallforprice');
        				$item->load($proId, 'product_id');
						$item->setProductId($proId);
						$item->setHidePrice('1')
							->setMessage($attributesData['mfm_cfp'])
							->save();
					} 
				}
				else {
