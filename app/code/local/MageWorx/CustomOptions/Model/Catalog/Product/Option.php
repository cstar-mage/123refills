...
class MageWorx_CustomOptions_Model_Catalog_Product_Option extends Mage_Catalog_Model_Product_Option {
    ...
    public function saveProductOptions(array $newOptions, array $prevOptions, array $productIds, Varien_Object $group, $prevGroupIsActive = 1, $remove = true, $place = 'apo') {        
        if (isset($productIds) && is_array($productIds)) {            
            $relation = Mage::getResourceSingleton('customoptions/relation');            
...
//            exit;                        
            ...
            if (isset($newOptions) && is_array($newOptions)) {
                foreach ($productIds as $productId) {                
                    ...
                    if (isset($optionRelations[$productId])) {
                        foreach ($optionRelations[$productId] as $prevOption) {
                            ...
                            if (isset($options['IGI'.$prevOption->getInGroupId()]) && (!isset($options['IGI'.$prevOption->getInGroupId()]['is_delete']) || $options['IGI'.$prevOption->getInGroupId()]['is_delete']!=1)) {
                                ...
                                $this->saveOption($productId, $diffOption, $optionId, $option['type']);                                
                    ...
                    foreach ($options as $option) {
                        ...
                        $optionId = $this->saveOption($productId, $option, false, $option['type']);
    ...
    public function saveOption($productId, $option, $optionId = 0, $type = '') {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $tablePrefix = (string) Mage::getConfig()->getTablePrefix();
...
//        );
    ...
    public function getIsRequire() {
        ...
        if (       ($moduleName=='checkout' && Mage::app()->getRequest()->getActionName()!='configure') 
                || ($moduleName=='wishlist' && Mage::app()->getRequest()->getActionName()!='configure') 
                || ($moduleName=='icart' && Mage::app()->getRequest()->getMethod()=='POST')
                || ($moduleName=='gomage_checkout')        
                || ((Mage::app()->getRequest()->getControllerName()=='sales_order_edit' || Mage::app()->getRequest()->getControllerName()=='sales_order_create') &&  Mage::app()->getRequest()->getActionName()=='loadBlock')
           ) {
