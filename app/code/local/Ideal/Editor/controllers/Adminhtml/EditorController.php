<?php
class Ideal_Editor_Adminhtml_EditorController extends Mage_Adminhtml_Controller_action
{
	...
	public function importcssAction(){
		$skinpath = Mage::getDesign()->getSkinBaseDir(array('_area'=>'frontend','_package'=>'evolved'));
		$dirPath = $skinpath. DS ."css". DS;
		$data = Mage::getModel('editor/editor');
		$collection = $data->cssCollection($dirPath);
		//echo "<pre>";
		foreach($collection as $col){
			$ext = substr($col, strrpos($col, '.') + 1);
			if($ext != 'css') {
				continue;
			}
			$data1 = Mage::getModel('editor/editor');
 			$data1->setTitle($col);
 			$data1->setContent(file_get_contents($dirPath.$col));
 			$data1->save();
			//echo "<br />".$dirPath.$col."<br />";
			//print_r(file_get_contents($dirPath.$col));
		}
		//exit;
		Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('editor')->__('Css loaded successfully'));
		$this->_redirect('*/*/');
	}
    ...
    {
    	return Mage::getSingleton('admin/session')->isAllowed('theme/editor');
    }
}
