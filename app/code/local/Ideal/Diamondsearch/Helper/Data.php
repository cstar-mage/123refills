...
class Ideal_Diamondsearch_Helper_Data extends Mage_Core_Helper_Abstract
{
	function getDiamondsArray($stones, $is_fancy, $is_specialshape)
	{
		...
		$images_flag = $this->getImagesExistFlag(); // getting lot#s of correct image URLs
		
		//CALL for price
		$cfp_data = $this->getCallForPriceData();
		$price = "CALL";

		$frontend_url = "diamondsearch";
		if( Mage::getStoreConfig("diamondsearch/general_settings/frontend_url") )
			$frontend_url = Mage::getStoreConfig("diamondsearch/general_settings/frontend_url");
		...
		foreach ( $stones as $stone) {
			...
			//$link = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).$frontend_url.DS.$stone['carat']."-".$stone['shape']."-".$stone['cut']."-".$stone['certificate']."-".$color."-".$stone['clarity']."-diamond-stock-".$stone['lotno']."-cert-".$stone['cert_number'] ;
			$link = $this->getDiamondLink($stone['carat'],$stone['shape'],$stone['cut'],$stone['certificate'],$color,$stone['clarity'],$stone['lotno'],$stone['cert_number']);
			
			$diamond_image = false;
			$is_samp = false;
			if(in_array($stone['lotno'], $images_flag)) $diamond_image = $stone['diamond_image'];
			else {
				if(file_exists(Mage::getBaseDir('media'). DS . "dsearch/samples" . DS .strtolower($stone['shape'])."_pic_large.jpg")){
					$diamond_image = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . DS . "dsearch/samples" . DS .strtolower($stone['shape'])."_pic_large.jpg";
					$is_samp = true;
				}
				else $diamond_image = false;
			}
			/*
			if($stone['image_flag']==1 && trim($stone['diamond_image'])!="") $diamond_image = $stone['diamond_image'];
			else $diamond_image = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . DS . "dsearch/shape/270x270" . DS .strtolower($shape)."_pic_large.jpg";
			//Mage::getBaseDir('media')
			*/
			
			$cut = $this->getFullNameFromAbbreviation("cut", strtoupper($stone['cut']));
			...
			$diamonds[] = array(
				...
				"image" => $diamond_image,
				"is_samp" => $is_samp,
				"video" => $stone['image_or_video_link'] ? $stone['image_or_video_link'] : false,
	...
	function getImagesExistFlag(){
		...
		$result = mysql_query("SELECT lotno,image_flag FROM `$uploadtool_diamonds_images` where image_flag=1");  
		$storeArray = Array();
		while ($row = mysql_fetch_array($result, MYSQL_ASSOC))  
		{
			$storeArray[] = $row['lotno'];
		}
		return  $storeArray;
	}
	function getDiamondLink($carat="",$shape="",$cut="",$certificate="",$color="",$clarity="",$lotno="",$cert_number="",$frontend_url="")
	{
		if($frontend_url==""){
			$frontend_url = "diamondsearch";
			if( Mage::getStoreConfig("diamondsearch/general_settings/frontend_url") )
		...
		return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).$frontend_url.DS.$carat."-".$shape."-".$cut."-".$certificate."-".$color."-".$clarity."-diamond-stock-".$lotno."-cert-".$cert_number;
	}
		...
		//$resultHlml = '<div class="container top-tweny-list" style="">';
			...
			$resultHlml .= '<div class="mobile-results-table row">';
				...
				foreach ( $stones as $stone) {
					...
					$resultHlml .= '<tr data-ee-data="'.$eedata.'" class="ee-diamond" onclick="setLocation(\''.Mage::getUrl("diamondsearch/index/view", array("id"=>$stone['id'])).'\')">';
					$resultHlml .= '<th rowspan="2" scope="row">';
					$resultHlml .= '<a href="'.Mage::getUrl("diamondsearch/index/view", array("id"=>$stone['id'])).'">';
					...
					$resultHlml .= '<td rowspan="2"><a href="'.Mage::getUrl("diamondsearch/index/view", array("id"=>$stone['id'])).'" class="glyphicon glyphicon-play"></a></td>';
					$resultHlml .= '</tr>';
					$resultHlml .= '<tr onclick="setLocation(\''.Mage::getUrl("diamondsearch/index/view", array("id"=>$stone['id'])).'\')">';
	...
	function getFullNameFromAbbreviation($columnName, $abbr)
	{
		$newAbbr = array();
		$base["cut"] = array("I" => "IDEAL", "EX" => "EXCELLENT", "VG" => "VERY GOOD", "G" => "GOOD", "F" => "FAIR");
		$base["fluorescence"] = array("VS" => "VERY STRONG", "S" => "STRONG", "M" => "MEDIUM", "F" => "FAINT", "N" => "NONE");
		
		if(array_key_exists($abbr, $base[$columnName]))return $base[$columnName][$abbr];
		else return $abbr;
	...
	function getTemplatePath()
	{
		...
		//$skin = "diamondsearch/diamondsearch-grid.phtml";
