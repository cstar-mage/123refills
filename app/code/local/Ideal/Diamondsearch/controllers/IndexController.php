<?php
class Ideal_Diamondsearch_IndexController extends Mage_Core_Controller_Front_Action
{
    public function viewAction()
    {
		...
		if ($block){//check if block actually exists
			if(Mage::getStoreConfig("diamondsearch/general_settings/spacial_diamond_avilability")==1){
				$block->setTemplate("diamondsearch/diamondview-special.phtml"); 
			}
			else {
				$block->setTemplate("diamondsearch/diamondview.phtml"); 
			}
		}
...
function manageZoom() {
	...
	if(hasClass(el, 'out')) {
		...
		{
			echo "<h3>CERTIFICATE NOT AVAILABLE</h3>";
		}
