...
class Jewelerslink_Jewelryshare_Block_Adminhtml_Edit_Codes_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _getImportCodeList()
    {
    	...
    	try
    	{
    		...
    		$ch = curl_init();
    		$timeout = 5;
    		curl_setopt($ch,CURLOPT_URL,"http://www.jewelerslink.com/jewelry/index/getcolumnnames");
    ...
    protected function _prepareForm()
    {
        ...
        if ( Mage::getSingleton('adminhtml/session')->getJewelryshareCodesData() )
        {
        	$form->setValues(Mage::getSingleton('adminhtml/session')->getJewelryshareCodesData());
        	Mage::getSingleton('adminhtml/session')->setJewelryshareCodesData(null);
        } elseif ( Mage::registry('jewelryshare_codes_data') ) {
        	$form->setValues(Mage::registry('jewelryshare_codes_data')->getData());
        }
