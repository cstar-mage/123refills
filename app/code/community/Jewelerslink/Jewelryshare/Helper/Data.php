...
class Jewelerslink_Jewelryshare_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getImportCSV() {
		try
		{
			...
			if($data == "Invalid Login") {
				...
				return array("success"=>0 ,"message"=> "Unauthenticate Login, Go to ( System > Configuration > Jewelry Config ) and enter Jewelerslink Login Detail");
				...
				foreach($csvData as $column) {
					...
					if(isset($csvData[$csvRowCnt]['gender']) && $csvData[$csvRowCnt]['gender'] == "His") { 
						$csvData[$csvRowCnt]['gender'] = 'Male';
					}
					
					if(isset($csvData[$csvRowCnt]['metal_type'])) 
					{ 
						if($csvData[$csvRowCnt]['metal_type']=='14K Two-Tone Gold') { $csvData[$csvRowCnt]['metal_type']='Two Tone'; } 
						if($csvData[$csvRowCnt]['metal_type']=='10K White Gold') { $csvData[$csvRowCnt]['metal_type']='10k White Gold'; } 
						if($csvData[$csvRowCnt]['metal_type']=='10K Yellow Gold') { $csvData[$csvRowCnt]['metal_type']='10k Yellow Gold'; } 
						if($csvData[$csvRowCnt]['metal_type']=='14K White Gold') { $csvData[$csvRowCnt]['metal_type']='14k White Gold'; } 
						if($csvData[$csvRowCnt]['metal_type']=='14K Yellow Gold') { $csvData[$csvRowCnt]['metal_type']='14k Yellow Gold'; } 
						if($csvData[$csvRowCnt]['metal_type']=='14k Rose Gold') { $csvData[$csvRowCnt]['metal_type']='14k Rose Gold'; } 
					}
		...
		try {
			...
			while($csv_line = fgetcsv($fp,10240))
			{
				...
				if(isset($imageKey) && $imageKey != "") {
					...
					$httpUrl = str_replace(" ","%20","http://images.jewelerslink.com/jewelry/".$imagePath);
				...
				if(isset($small_imageKey) && $small_imageKey != "") {
					...
					$shttpUrl = str_replace(" ","%20","http://images.jewelerslink.com/jewelry/".$simagePath);
				...
				if(isset($thumbnailKey) && $thumbnailKey != "") {
					...
					$thttpUrl = str_replace(" ","%20","http://images.jewelerslink.com/jewelry/".$timagePath);
				...
				if(isset($galleryKey) && $galleryKey != "") {
					...
					foreach($galleryArray as $galleryImg) {
						...
						$ghttpUrl = str_replace(" ","%20","http://images.jewelerslink.com/jewelry/".$gimagePath);
	...
	public function importProducts() {
		...
		$filename = Mage::app()->getRequest()->setParam('files','jewelerslink_import.csv');
	...
	public function getUpdateCSV() {
		try
		{
			...
			$ch = curl_init();
			$timeout = 5;
			curl_setopt($ch,CURLOPT_URL,"http://www.jewelerslink.com/jewelry/index/getMappedUpdateJson");
			curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
			curl_setopt($ch,CURLOPT_CONNECTTIMEOUT,$timeout);
			curl_setopt($ch, CURLOPT_POSTFIELDS, array("username"=>$username,"password"=>$password,"vendors"=>$data_string,"attributes"=>$mapped_string));
			$data = curl_exec($ch);
	...
	public function updateProducts() {
		...
		$filename = Mage::app()->getRequest()->setParam('files','jewelerslink_update.csv');
		$filename = Mage::app()->getRequest()->getParam('files'); // set the filename that is to be imported - file needs to be present in var/import directory
