...
/**
 * TheJewelerslink watches import model
 *
 * @category    Jewelerslink
 * @package     Jewelerslink_Watches
 */
class Jewelerslink_Watches_Model_Importcron extends Mage_Core_Model_Abstract
{
    //const SEPARATOR = "\t";
    const LINE_END  = "\r\n";
    const ENCLOSURE = '"';
    ...
    const XML_PATH_SETTINGS_FTP_SERVER           = 'watches/settings/ftp_server';
    const XML_PATH_SETTINGS_FTP_USER             = 'watches/settings/ftp_user';
    const XML_PATH_SETTINGS_FTP_PASSWORD         = 'watches/settings/ftp_password';
    //const XML_PATH_SETTINGS_FTP_PATH             = 'watches/settings/ftp_path';
    //const XML_PATH_SETTINGS_FINDFEED_FILENAME    = 'watches/settings/jewelerslinkwatches_filename';
    const XML_NODE_FIND_FEED_ATTRIBUTES = 'watches_attributes';
    ...
    /**
     * TheJewelerslink watches process import
     */
    public function processImport()
    {
        ...
        if ($file) {
            ...
            if (!$this->_deleteFile($file)) {
                Mage::throwException(Mage::helper('watches')->__("FTP: Can't delete files"));
            }
        }
    }
    ...
    protected function _createFile()
    {
        ...
        for ($i = 1; $i <= $pageNumbers; $i++) {
            ...
            $productCollection->addAttributeToFilter('watch_imported', 1);
    ...
    {
        $attributes = Mage::getResourceModel('jewelerslink_watches/codes_collection')
          ->getImportAttributes();

        if (!Mage::helper('watches')->checkRequired($attributes)) {
            return false;
        }
        return $attributes;
    }
    ...
    /**
     * Delete all files in current watches ftp directory
     *
     * @return bool
     */
    ...
    {
        return Mage::getBaseDir('var') . DS . 'export' . DS . 'watches' . DS;
    }
    ...
    public function getJwCustomerId()
    {
    	try
    	{
    		...
    		if($data == "Invalid Login") {
    			//echo "Unauthenticate Login, Go to ( System > Configuration > Watches Config ) and enter Jewelerslink Login Detail";
    			...
    			echo "Unauthenticate Login, Go to ( System > Configuration > Watches Config ) and enter Jewelerslink Login Detail"; exit;
    			return;
    		} else {
    			//echo $data; exit;
    			return $data;
