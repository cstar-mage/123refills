 ...
 */
class ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Salesreport_Leastviewed
    extends
    ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Salesreport_Abstract
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->setType(
            'dyncatprod/rule_condition_additional_conditions_salesreport_leastviewed'
        )
            ->setValue(null)
            ->setConditions(array())
            ->setActions(array());
    ...
    public function asHtml()
    {
        $html = $this->getTypeElement()->getHtml() .
            Mage::helper('dyncatprod')->__(
                "Least Viewed Products from the most viewed report %s %s day(s) before the day the category is viewed",
    ...
    public function asString($format = '')
    {
        $str = Mage::helper('dyncatprod')->__(
            "Least Viewed Products from the most viewed report <strong>%s %s</strong> day(s) before the day the category is viewed",
    ...
    public function _validate(Varien_Object $object)
    {
        try {
            ...
            if ($value != '0' || $value != '') {
                ...
                $customCollection->addViewsCount(
                ...
                $this->getHelper()->debug(
                    'LEAST VIEWED SQL: ' . $customCollection->getSelect()
                );
