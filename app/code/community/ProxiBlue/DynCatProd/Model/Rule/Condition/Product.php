    ...
    extends ProxiBlue_DynCatProd_Model_Rule_Condition_Product_Abstract
{
    ...
    {
        /**
         * Render attributes display.
         * Some have special rendering so deal with them.
         *
         */
    ...
    public function getDefaultOperatorOptions()
    {
        if (null === $this->_defaultOperatorOptions) {
            $this->_defaultOperatorOptions = array(
                ...
                '!!'  => Mage::helper('rule')->__(
                    'is within range on day of viewing'
                ),
                '!!!' => Mage::helper('rule')->__(
                    'is not within range on day of viewing'
                ),
                '<D'  => Mage::helper('rule')->__(
                    'is X days ago on day of viewing'
                ),
                '==|' => Mage::helper('rule')->__(
                    'is ( ignore attribute default )'
                ),
                '!=|' => Mage::helper('rule')->__(
                    'is not ( ignore attribute default )'
                ),
            );
        }
    ...
    public function getValueAfterElementHtml()
    {
        ...
        switch ($this->getAttribute()) {
            ...
            case 'applied_catalog_rule_id':
                ...
                $html
                    .= '<a href="javascript:void(0)" class="rule-chooser-trigger"><img src="'
                    . Mage::getDesign()
                        ->getSkinUrl('images/rule_chooser_trigger.gif')
    ...
    {
        $opt = array();
        foreach ($this->getAttributeOption() as $value) {
