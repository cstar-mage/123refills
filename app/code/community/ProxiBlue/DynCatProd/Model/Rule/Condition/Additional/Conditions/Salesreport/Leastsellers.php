 ...
 */
class ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Salesreport_Leastsellers
    extends
    ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Salesreport_Abstract
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->setType(
            'dyncatprod/rule_condition_additional_conditions_salesreport_leastsellers'
        )
            ->setValue(null)
            ->setConditions(array())
            ->setActions(array());
    ...
    public function asHtml()
    {
        $html = $this->getTypeElement()->getHtml() .
            Mage::helper('dyncatprod')->__(
                "Least selling products from the best sellers sales report %s %s day(s) before the day the category is viewed",
    ...
    public function asString($format = '')
    {
        $str = Mage::helper('dyncatprod')->__(
            "Least selling products from the best sellers sales report <strong>%s %s</strong> day(s) before the day the category is viewed",
        ...
        try {
            ...
            if ($value != '0' || $value != '') {
                ...
                $this->getHelper()->debug(
                    'LEAST SELLERS SQL: ' . $customCollection->getSelect()
                );
