 ...
 */
class ProxiBlue_DynCatProd_Model_Rule_Condition_Category_Conditions_Category
    extends ProxiBlue_DynCatProd_Model_Rule_Condition_Product_Abstract
{
    ...
    public function validate(Varien_Object $object)
    {
        ...
        if ($currentValue != $this->getValue()) {
            ...
            if ($object->getIsDynamicCronRun()
                && mage::getStoreConfig(
                    'dyncatprod/notify/category_disabled'
                )
                && $this->getAttribute() == 'is_active'
                && $this->getValue() == 0
            ) {
                $message = Mage::helper('core')->__(
                    "Category '%s' (%s) was disabled.", $object->getName(),
                    $object->getId()
                );
                mage::helper('dyncatprod')->sendEmail(
                    Mage::helper('core')->__(
                        "Dynamic Category Products Category Control Notification for %s",
                        $object->getName()
                    ), $message
                );
            }
    ...
    public function getAttributeObject()
    {
        try {
            $obj = Mage::getSingleton('eav/config')
                ->getAttribute(
                    Mage_Catalog_Model_Category::ENTITY, $this->getAttribute()
                );
        } catch (Exception $e) {
