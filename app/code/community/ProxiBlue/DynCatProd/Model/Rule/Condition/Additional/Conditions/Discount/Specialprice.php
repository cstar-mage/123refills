 ...
 */
class ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Discount_Specialprice
    extends
    ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Abstract
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->setType(
            'dyncatprod/rule_condition_additional_conditions_discount_specialprice'
        )
            ->setValue(null)
            ->setConditions(array())
            ->setActions(array());
    ...
    public function asHtml()
    {
        $html = $this->getTypeElement()->getHtml() .
            Mage::helper('dyncatprod')->__(
                "If a product, attached the website(s) %s, has a special price value set
                 that gives %s %s off the product price",
    ...
    public function asString($format = '')
    {
        $str = Mage::helper('dyncatprod')->__(
            "If a product has a special price value set that gives <strong>%s %s</strong> off the product price",
    ...
    public function validate(Varien_Object $object)
    {
        ...
        $this->getHelper()->debug(
            'SPECIAL PRICE SQL BEFORE: ' . $collection->getSelect(), 5
        );
        ...
        if (strpos($value, '%') > 0) {
            ...
            $select->where(
                '( 100 - (( ' . $helper->getColumnName(
                    $select->getPart(Zend_Db_Select::COLUMNS), 'special_price'
                )
                . '.value * 100 ) / ' . $helper->getColumnName(
                    $select->getPart(Zend_Db_Select::COLUMNS), 'price'
                )
        ...
        if (is_array($this->getWebsites()) && count($this->getWebsites()) > 0) {
            $collection->addWebsiteFilter($this->getWebsites());
        }
        $this->getHelper()->debug(
        ...
        $this->getHelper()->debug(
            'SPECIAL PRICE SQL Adjusted: ' . $collection->getSelect()
        );
