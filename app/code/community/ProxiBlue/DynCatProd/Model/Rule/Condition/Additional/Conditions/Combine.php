 ...
 */
class ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Combine
    extends ProxiBlue_DynCatProd_Model_Rule_Condition_Backport
{
     ...
     * This should not happen, but can is someone ads two of the same identical
     * rules (like two limiters)
     *
     * @return type
     */
    public function getTypeElement()
    {
        try {
            return $this->getForm()->addField(
                $this->getPrefix() . '__' . $this->getId() . '__type', 'hidden',
                array(
                    'name'    =>
            ...
            return $this->getForm()->addField(
                $this->getPrefix() . '__' . $this->getId() . '__' . rand(1, 100)
                . '__type', 'hidden', array(
                    'name'    =>
                        'rule[' . $this->getPrefix() . '][' . $this->getId()
                        . '][type]',
