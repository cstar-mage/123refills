 ...
 */
class ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Limiter
    extends
    ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Abstract
{
    ...
    public function __construct($args)
    {
        ...
        $this->setType(
            'dyncatprod/rule_condition_additional_conditions_limiter'
        )
            ->setProcessingOrder(99999999999999);
    }
        ...
        try {
            $html = $this->getTypeElement()->getHtml() .
                Mage::helper('dyncatprod')->__(
                    "Limit the result to %s %s products",
    ...
    public function asString($format = '')
    {
        $str
            = Mage::helper('dyncatprod')->__(
            "Limit the result to <strong>%s</strong> <strong>%s</strong> products",
    ...
    public function validate(Varien_Object $object)
    {
        ...
        switch ($operator) {
            case 'LAST':
                $collection->getSelect()->order(
                    'e.entity_id ' . Zend_Db_Select::SQL_DESC
                );
                break;
            case 'OFFSET':
        ...
        $this->getHelper()->debug(
            'LIMITER SQL ADJUSTED: ' . $collection->getSelect()
        );
