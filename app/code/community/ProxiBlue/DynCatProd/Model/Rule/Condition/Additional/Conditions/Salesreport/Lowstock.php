 ...
 */
class ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Salesreport_Lowstock
    extends
    ProxiBlue_DynCatProd_Model_Rule_Condition_Additional_Conditions_Salesreport_Abstract
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->setType(
            'dyncatprod/rule_condition_additional_conditions_salesreport_lowstock'
        )
            ->setValue(null)
            ->setConditions(array())
            ->setActions(array());
    ...
    public function asHtml()
    {
        $html = $this->getTypeElement()->getHtml() .
            Mage::helper('dyncatprod')->__(
                "Products from the low stock report where the stock level is %s %s on the day the category is viewed",
    ...
    public function asString($format = '')
    {
        $str = Mage::helper('dyncatprod')->__(
            "Products from the low stock report where the stock level is <strong>%s %s</strong> on the day the category is viewed",
    ...
    public function _validate(Varien_Object $object)
    {
        try {
            ...
            $this->getHelper()->debug(
                'LOW STOCK SQL: ' . $customCollection->getSelect()
            );
    ...
    protected function getCollection()
    {
        ...
        $collection = Mage::getResourceModel(
            'reports/product_lowstock_collection'
        )
        ...
        $collection->getSelect()->where(
            'qty ' . $this->_operatorMapToSql[$this->getOperator()] . ' ?',
            (int)$this->getValue()
        );
        if ($storeId) {
