 ...
 */
class ProxiBlue_DynCatProd_Model_Rule_Condition_Product_Combine
        ...
        foreach ($productAttributes as $type => $attributeData) {
            foreach ($attributeData as $code => $label) {
                if (strpos($code, 'quote_item_') !== 0) {
                    $pAttributes[$type][] = array('value' =>
                                                      'dyncatprod/rule_condition_product|'
                                                      . $code,
                                                  'label' => $label);
                }
            }
        ...
        $conditions = array_merge_recursive(
            $conditions, array(
                array('label' => Mage::helper('catalog')->__(
                    ...
                    array('value' => 'dyncatprod/rule_condition_additional_conditions_salesreport',
                          'label' => Mage::helper('dyncatprod')->__(
                              'Using Sales Reports'
                          )),
                    array('value' => 'dyncatprod/rule_condition_additional_conditions_tags',
                          'label' => Mage::helper('dyncatprod')->__(
                              'Using Product Tags'
                          )),
                )),
    ...
    public function addCondition($condition)
    {
        ...
        if ($condition->getProcessingOrder() === false
            ...
            mage::helper('dyncatprod')->debug(
                ...
                . ' does not have a processing order set. This can result in unexpected rules behaviour. Contact sales@proxiblue.com.au to correct this.'
            );
        ...
        if ($condition->getProcessingOrder() == 'first') {
            ...
            if (array_key_exists(
                $condition->getProcessingOrder(), $conditions
            )) {
                mage::helper('dyncatprod')->debug(
                    'Rule ' . get_class($condition)
                    . ' appear to have the same processing order than another rule. The rule order process was increased by 1 to try and allow the system to continue.'
                );
                $condition->setProcessingOrder(
                    $condition->getProcessingOrder() + 1
                );
            }
