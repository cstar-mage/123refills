...
class Unirgy_StoreLocator_Model_Mysql4_Location_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function addAreaFilter($storetype, $center_lat, $center_lng, $radius, $units='mi')
    {
        ...
        $this->_select = $conn->select()->from(array('main_table' => $this->getResource()->getMainTable()), array('*', 'distance'=>$dist))
            ->where('`latitude` is not null and `latitude`<>0 and `longitude` is not null and `longitude`<>0 and `store_name` LIKE "%'.$storetype.'%" and '.$dist.'<=?', $radius)
            ->order('distance');
        /* echo $this->_select;
        exit; */
        return $this;
