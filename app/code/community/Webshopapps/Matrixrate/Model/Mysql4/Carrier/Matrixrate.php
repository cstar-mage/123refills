...
//            }
            ...
            if (!empty($csv)) {
                ...
                if (empty($exceptions)) {
                    ...
                    foreach ($csvLines as $k=>$csvLine) {
						...
						if (count($csvLine)==12) {
							...
							$data[] = array('website_id'=>$websiteId, 'dest_country_id'=>$countryId, 'dest_region_id'=>$regionId, 'dest_city'=>$city, 'dest_zip'=>$zip, 'dest_zip_to'=>$zip_to, 'condition_name'=>$csvLine[9], 'per_lb_cost'=>$csvLine[10], 'condition_from_value'=>$csvLine[5],'condition_to_value'=>$csvLine[6], 'price'=>$csvLine[7], 'delivery_type'=>$csvLine[8],'ship_code'=>$csvLine[11]);
                ...
                if (empty($exceptions)) {
                    ...
                    //echo "DELETE FROM `".$table."` where website_id = '".$websiteId."' AND condition_name IN ('package_value','package_weight')"; exit;
                    //$connection->query("DELETE FROM `".$table."` where website_id = '".$websiteId."' AND condition_name IN ('package_value','package_weight')");
                    ...
                    foreach($data as $k=>$dataLine) {
                        try {
                           $connection->insert($table, $dataLine);
                        	...
                        	//removed as we are using combined matrix
