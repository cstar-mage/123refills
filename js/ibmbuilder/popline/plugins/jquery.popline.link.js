/*
  ...
  */
      ...
      $textField.keyup(function(event) {
        if (event.which === 13) {
          ...
          if (pattern.test(text)) {
            ...
            if (isUnremovable()) {
              var selectionWrapper = getSelectionWrapper();
              selectionWrapper.setAttribute('href', text);
            } else {
  ...
  // Some links can`t be removed. It`s important for js logic and styles.
  var getSelectionWrapper = function() {
    return $.popline.utils.selection().range().commonAncestorContainer.parentNode;
  }

  var isUnremovable = function() {
    var selectionWrapper = getSelectionWrapper();

    var isDisabled = false;
    $.each(selectionWrapper.attributes, function(index, element) {
  ...
  $.popline.addButton({
    link: {
      ...
      beforeShow: function(popline) {
        ...
        if ($.popline.utils.selection().focusNode().nodeType != 3) {
          $a.hide();
          return;
        } else {
          $a.show();
        ...
        if (selectionIsLink() && selectionWrapper.getAttribute('href') != '#') {
        ...
        if (!this.data("click-event-binded")) {
          ...
          this.click(function(event) {
            ...
            if (selectionIsLink() && selectionWrapper.getAttribute('href') != '#') {
              ...
              if (isUnremovable()) {
                selectionWrapper.setAttribute("href", "#");
              } else {
