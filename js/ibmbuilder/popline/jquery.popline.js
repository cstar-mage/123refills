...
;(function ($) {
    ...
    $(document).on('click', '.export_blocks', function (event) {
        $.popline.hideAllBar();
    });
    ...
    var toggleBox = function (event) {
        ...
        if ((isTargetOrChild || isBarOrChild) && $.popline.utils.selection().text().length > 0 && !$.popline.current.keepSilentWhenBlankSelected()) {
            ...
            if ($.popline.current.bar.find('.dont_hide_on_click').length > 0) {
                return;
            }
            $.popline.hideAllBar();
        }
    ...
    var Position = function () {
        ...
        var positions = {
            ...
            "relative": {
                mouseup: function (event) {
                    var editedBlock = $('#frame .current_editing');
                    var blockOffsetLeft = editedBlock.offset().left;
                    var blockOffsetTop = editedBlock.offset().top;
                    // var left = event.pageX - bar.width() / 2;
                    var barWidth = bar.width();
                    var blockWidth = editedBlock.width();
                    
                    var left = blockOffsetLeft + (blockWidth - barWidth) / 2;
                    if (left < 0) left = 10;
                    var scrollTop = $(document).scrollTop();
                    // var top = event.pageY - bar.outerHeight() - parseInt(target.css('font-size'));
                    var top = blockOffsetTop - parseInt(bar.height());
                    if (top < scrollTop) top = event.pageY + parseInt(target.css('font-size'));
                    /*$(".popline-button").find(".more").remove();
                     $(".popline").find(".popline-subbar").closest("li").append('<div class="more">');*/
                    $(".popline").find(".popline-subbar").prevAll(".popline-btn").addClass("ibm_subs");
                    return {left: left, top: top};
    ...
    $.extend($.popline, {
        ...
        prototype: {
            ...
            init: function () {
                ...
                var makeButtons = function (parent, buttons) {
                    for (var name in buttons) {
                        ...
                        if (button.directContent) {
                            $button.children(".popline-btn").append(button.directContent.content);
                        } else {
                        ...
                        if (button.content) {
                            $button.children(".popline-btn").append(button.content);
                        }
                        ...
                        $button.mousedown(function (event) {
                            if (!$(event.target).is("input") && !$(event.target).hasClass("dont_prevent_default")) {
            ...
            show: function (options) {
                if (typeof window.getSelection() != 'undefined') {
                    ...
                    $.popline.activeSelection = selection.getRangeAt(0);
                }
        ...
        getUniqueId: function() {
            if (isNaN(window['unique_popline_id'])) {
                window['unique_popline_id'] = 0;
            }

            return window['unique_popline_id']++;
        },
        
        selectCurrentEditedText: function() {
            // Select div`s text for changes applying
            var containerElement = $.popline.activeSelection;
            var text = containerElement.commonAncestorContainer;
            var selection = window.getSelection();
            var range = document.createRange();
            range.selectNodeContents(text);
            range.setStart(containerElement.startContainer, containerElement.startOffset);
            range.setEnd(containerElement.endContainer, containerElement.endOffset);
